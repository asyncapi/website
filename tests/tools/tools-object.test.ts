...
import {
  convertTools,
  createToolObject,
} from '../../scripts/tools/tools-object';
import {
...
describe('Tools Object', () => {
  ...
  const mockToolData = (
    toolContent: AsyncAPITool,
    toolNames = ['valid-tool'],
  ) => {
    const mockData = createMockData(
      toolNames.map((name) => ({
        name: `.asyncapi-tool-${name}`,
        repoName: name,
      })),
    );
  ...
  it('should skip files that do not start with .asyncapi-tool', async () => {
    const mockData = createMockData([
      { name: 'not-asyncapi-tool', repoName: 'non-tool-repo' },
    ]);
