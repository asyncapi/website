...
describe('buildTools', () => {
  ...
  it('should extract, convert, combine tools, and write to file', async () => {
    ...
    const automatedToolsContent = JSON.parse(
      fs.readFileSync(automatedToolsPath, 'utf8'),
    );
    ...
    expect(Object.keys(automatedToolsContent)).toEqual(
      Object.keys(mockConvertedData),
    );
    expect(automatedToolsContent.Category1.description).toEqual(
      mockConvertedData.Category1.description,
    );
    expect(automatedToolsContent.Category2.description).toEqual(
      mockConvertedData.Category2.description,
    );

    expect(combinedToolsContent).toHaveProperty('Category1');
    expect(combinedToolsContent).toHaveProperty('Category2');
    expect(combinedToolsContent.Category1.description).toEqual(
      mockConvertedData.Category1.description,
    );
    expect(combinedToolsContent.Category2.description).toEqual(
      mockConvertedData.Category2.description,
    );
  ...
  it('should handle file write errors', async () => {
    ...
    await expect(
      buildTools(invalidPath, manualToolsPath, toolsPath, tagsPath),
