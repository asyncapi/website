...
describe('rssFeed', () => {
  ...
  afterAll(async () => {
    try {
      ...
      await Promise.all(
        files.map((file) => fs.promises.unlink(path.join(testOutputDir, file))),
      );
  ...
  it('should prioritize featured posts over non-featured ones', async () => {
    ...
    const itemTitles = parsedContent.rss.channel.item.map(
  ...
  it('should set correct enclosure type based on image extension', async () => {
    ...
    expect(fileContent).toContain(
      '<enclosure url="https://www.asyncapi.com/img/test-cover.png"',
    );
    expect(fileContent).toContain('type="image/png"');
    expect(fileContent).toContain(
      '<enclosure url="https://www.asyncapi.com/img/test-cover.svg"',
    );
    expect(fileContent).toContain('type="image/svg+xml"');
    expect(fileContent).toContain(
      '<enclosure url="https://www.asyncapi.com/img/test-cover.webp"',
    );
  ...
  it('should catch and handle errors when write operation fails', async () => {
    ...
    await expect(rssFeed(type, title, desc, invalidOutputPath)).rejects.toThrow(
      /ENOENT|EACCES/,
    );
  });
  ...
  it('should use default mime type when encountering an unsupported image extension', async () => {
    ...
    expect(fileContent).toContain(
      '<enclosure url="https://www.asyncapi.com/img/test-cover.unknown"',
    );
