    ...
    try {
      if (tool.name.startsWith('.asyncapi-tool')) {
        ...
        if (isValid) {
          ...
          let toolObject = await createToolObject(
            jsonToolFileContent,
            repositoryUrl,
            repoDescription,
            isAsyncAPIrepo,
          ...
          jsonToolFileContent.filters.categories.forEach(async (category) => {
            ...
            if (categorySearch.length) {
              ...
              if (
                !finalToolsObject[searchedCategoryName].toolsList.find(
                  (element) => element === toolObject,
                )
              )
                finalToolsObject[searchedCategoryName].toolsList.push(
                  toolObject,
                );
            } else {
              // if Tool object has a category, not defined in our categorylist, then this provides a `other` category to the tool.
              if (
                !finalToolsObject['Others'].toolsList.find(
                  (element) => element === toolObject,
                )
              )
