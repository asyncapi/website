...
function walkDirectories(directories, result, basePath, sectionWeight = 0, sectionTitle, sectionId, rootSectionId) {
  for (let dir of directories) {
    const directory = path.normalize(dir[0]);
    ...
    for (let file of files) {
      ...
      const fileNameWithSection = path.join(fileName, '_section.mdx');
      const slug = fileName.replace(new RegExp(`^${basePath.replace(/\\/g, '\\\\')}`), '').replace(/\\/g, '/'); // Ensure forward slashes for slugs
      ...
      if (isDirectory(fileName)) {
        if (existsSync(fileNameWithSection)) {
          ...
          details.title = details.title || capitalize(path.basename(fileName));
        } else {
          details = { title: capitalize(path.basename(fileName)) };
        }
        ...
        if (details.slug.includes('/reference/specification/') && !details.title) {
          const fileBaseName = path.basename(data.slug);
