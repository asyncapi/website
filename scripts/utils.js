...
function checkAndGroupDocumentsByCategory(results) {
  ...
  return results.reduce((items, item, index) => {
    ...
    const { menu: { docs: { parent: documentCategoryName, parentWeight } = {} } = {} } = item;
    const documentCategory = items.find((item) => item.title === documentCategoryName);

    if(!documentCategory){
      const sectionWeight = parentWeight || index;
      items.push({
        title: documentCategoryName,
        isSection: true,
        weight: sectionWeight
      });
      item.sectionWeight = sectionWeight;
      item.sectionTitle = documentCategoryName;
    } else {
      item.sectionWeight = documentCategory.weight;
      item.sectionTitle = documentCategory.title;
    }
...
module.exports = {
  walkDirectories,
  checkAndGroupDocumentsByCategory
}