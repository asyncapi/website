...
AsyncAPI CLI maintains **two generator versions** to ensure compatibility across different template ecosystems. Understanding which version to use is crucial for successful generation.

### Which Generator Should I Use?

#### Use the **Default Generator (v1)** when:
- Working with **older templates** (published before 2023)
- Using templates that **don't explicitly mention** generator v2 support
- You get **errors with the new generator** and need fallback compatibility
- Working with **legacy projects** that require specific older template behavior

#### Use the **New Generator (v2)** with `--use-new-generator` flag when:
- Working with **newer templates** (version 3.0.0+, published 2023 or later)
- Template documentation **explicitly mentions** generator v2 support
- You need **improved performance** and modern features
- Working with **AsyncAPI v3.0+** documents
- Starting **new projects** (highly recommended)

### How to Identify the Right Generator

**Step 1: Check Template Version**
```bash
# Template version 3.0.0+ usually supports new generator
@asyncapi/html-template@3.0.0 → Use --use-new-generator
@asyncapi/html-template@2.x.x → Use default generator
```

**Step 2: Check Template Documentation**
- Look for mentions of "generator v2", "new generator", or "@asyncapi/generator v2"
- Check the template's README or package.json for generator requirements

**Step 3: Trial Method**
1. Try **without** `--use-new-generator` first
2. If you get errors like "template not compatible" or "unsupported features", add `--use-new-generator`
...
```bash
# NEW PROJECTS - Use new generator with modern templates
asyncapi generate fromTemplate asyncapi.yaml @asyncapi/html-template@3.0.0 --use-new-generator
asyncapi generate fromTemplate asyncapi.yaml @asyncapi/markdown-template@1.5.0 --use-new-generator

# LEGACY PROJECTS - Use default generator with older templates  
asyncapi generate fromTemplate asyncapi.yaml @asyncapi/html-template@2.3.0
asyncapi generate fromTemplate asyncapi.yaml @asyncapi/nodejs-template@1.0.0

# ASYNCAPI v3.0+ DOCUMENTS - Always use new generator
asyncapi generate fromTemplate asyncapi-v3.yaml @asyncapi/html-template@3.0.0 --use-new-generator
```

#### Common Mistakes

```bash
# DON'T: Using new generator with old templates
asyncapi generate fromTemplate asyncapi.yaml @asyncapi/html-template@2.0.0 --use-new-generator

# DON'T: Using old generator with new templates requiring v2
asyncapi generate fromTemplate asyncapi.yaml @asyncapi/html-template@3.0.0
```

### Troubleshooting

**Error: "Template requires generator v2"**
```bash
# Solution: Add --use-new-generator flag
asyncapi generate fromTemplate asyncapi.yaml your-template --use-new-generator
```

**Error: "Unsupported generator version"**
```bash
# Solution: Remove --use-new-generator flag (use default)
asyncapi generate fromTemplate asyncapi.yaml your-template
```

**Error: "Template not found or incompatible"**
```bash
# Solution: Check template version and documentation
npm view @asyncapi/html-template versions --json
```

### Template Compatibility Reference

| Template | Version | Recommended Generator | Command |
|----------|---------|----------------------|---------|
| @asyncapi/html-template | 3.0.0+ | New (v2) | `--use-new-generator` |
| @asyncapi/html-template | 2.x.x | Default (v1) | *(no flag)* |
| @asyncapi/markdown-template | 1.0.0+ | New (v2) | `--use-new-generator` |
| @asyncapi/nodejs-template | 1.x.x | Default (v1) | *(no flag)* |

> **Pro Tip:** When in doubt, **start with the default generator**. If you encounter compatibility issues, then try adding `--use-new-generator`.

> **Best Practice:** For **new projects**, always use the **new generator (v2)** with modern templates for better performance and features.
...
_See code: [src/commands/config/index.ts](https://github.com/asyncapi/cli/blob/v2.16.4/src/commands/config/index.ts)_
...
_See code: [src/commands/config/context/add.ts](https://github.com/asyncapi/cli/blob/v2.16.4/src/commands/config/context/add.ts)_
...
_See code: [src/commands/config/context/init.ts](https://github.com/asyncapi/cli/blob/v2.16.4/src/commands/config/context/init.ts)_
...
_See code: [src/commands/config/context/use.ts](https://github.com/asyncapi/cli/blob/v2.16.4/src/commands/config/context/use.ts)_
...
_See code: [src/commands/convert.ts](https://github.com/asyncapi/cli/blob/v2.16.4/src/commands/convert.ts)_
