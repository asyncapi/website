import React from 'react';

import type { Expenses } from '@/types/FinancialSummary/BarChartComponent';

import Card from './Card';

/**
 * @description ExpensesCard component displays the supplied expenses for each month.
 */
export default function ExpensesCard({ expenses }: { expenses: Expenses }) {
  return (
    <div className='overflow-x-auto'>
      <div className='grid auto-cols-max grid-flow-col gap-4 p-4'>
        {Object.entries(expenses).map(([month, data]) => {
          return <Card key={month} month={month as keyof Expenses} data={data} />;
        })}
      </div>
    </div>
  );
}
