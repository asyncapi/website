...
export default function DemoAnimation({ className = '' }: IDemoAnimationProps) {
  ...
  const [showChannelsAndOperation, setShowChannelsAndOperation] =
    useState(false);
  const [showUntilMessagePayload, setShowUntilMessagePayload] = useState(false);
  const [showDisplayName, setShowDisplayName] = useState(false);
  const [showEmail, setShowEmail] = useState(false);
  const [showDisplayNameDescription, setShowDisplayNameDescription] =
    useState(false);
  ...
  function renderInfoBlock(callback: () => void) {
    ...
    const common = (
      <>
        ...
        <div>
          <span className="text-teal-400">&nbsp;&nbsp;title:</span> Account
          Service
        </div>
        <div>
      ...
      return (
        <>
          ...
          <Typing
            typingDelay={typingDelay}
            cursor={<Cursor />}
            onTypingDone={descriptionCallback}
          >
            <div>
      ...
      >
        <div className="z-20 mb-2 md:mb-0 md:mr-1 md:flex-1">
          <MacWindow
            ...
            title="asyncapi.yaml"
          >
            {(showEmailDescription || started) &&
              renderInfoBlock(() => setShowInfo(true))}
            {showInfo &&
              renderChannelsOperationBlock(() =>
                setShowChannelsAndOperation(true),
              )}
            {showChannelsAndOperation &&
              renderUntilMessagePayload(() => setShowUntilMessagePayload(true))}
            {(showUntilMessagePayload || showEmail) &&
              renderDisplayName(() => setShowDisplayName(true))}
            {(showDisplayName || showDisplayNameDescription) &&
              renderEmail(() => setShowEmail(true))}
          </MacWindow>
        </div>
        <div
          className={
            'relative z-10 transition-all duration-500 ease-in-out md:mb-0 md:ml-6 md:flex-1'
          }
        >
          <div
          ...
          >
            ...
            <p className="font-normal tracking mx-auto mb-6 max-w-3xl text-lg text-gray-700 dark:text-white">
              Open this example on AsyncAPI Studio to get a better taste of the
              specification. No signup is required!
            </p>
            <OpenInStudioButton />
          </div>
          <MacWindow
            className={`min-h-full border border-gray-200 dark:border-dark-purple-700 bg-gray-50 dark:bg-dark-purple-900 shadow-lg transition-all duration-500 ease-in-out ${showControls ? 'h-0 -translate-x-full opacity-0 lg:h-auto lg:-translate-x-3/4 lg:opacity-100' : ''}`}
            contentClassName="text-left h-full text-gray-800 dark:text-gray-200 text-sm font-medium transition-all duration-500 ease-in-out"
          ...
          >
            ...
            <div className={transitionClassNames(showChannelsAndOperation)}>
              <span className="mr-2 rounded bg-green-500 px-3 py-2 font-bold text-white">
                RECEIVES
              </span>{' '}
            ...
            <div className={transitionClassNames(showUntilMessagePayload)}>
              <div className="mb-2 mt-4 text-gray-400">
                Accepts the following message:
              </div>
              <div className="rounded bg-gray-200 dark:bg-dark-purple-800 p-4 text-gray-600 dark:text-white">
                Payload{' '}
                <ArrowRight
                ...
                <div>
                  <div className="mt-2 rounded bg-gray-100 dark:bg-dark-purple-700 p-4">
                    <div
                      className={`mb-4 grid grid-cols-2 ${transitionClassNames(showDisplayName)}`}
                    >
                      <div>displayName</div>
                      <div>
                        <div className="font-bold text-green-500">String</div>
                        <div
                          className={`text-sm ${transitionClassNames(showDisplayNameDescription)}`}
                        >
                          Name of the user
                        </div>
                      </div>
                    ...
                    <div
                      className={`grid grid-cols-2 ${transitionClassNames(showEmail)}`}
                    >
                      <div>email</div>
                      <div>
                        <div className="font-bold text-green-500">
                          String
                          <span className="ml-2 rounded bg-yellow-300 px-1 py-0.5 text-xs text-black">
                            email
                          </span>
                        </div>
                        <div
                          className={`text-sm ${transitionClassNames(showEmailDescription)}`}
                        >
                          Email of the user
                        </div>
                      </div>
