 ...
 */
export default function ToolingCard({ name, description, badge, language, languageColor, link, isShuffling }: ToolingCardProps) {
  const CardContent = (
    <div className='relative max-w-xl w-full'>
      {/* Stacked Cards Behind - Positioned on LEFT side */}
      <div 
        className={`absolute w-full h-full bg-gradient-to-br from-purple-200 to-purple-300 dark:from-purple-800/60 dark:to-purple-700/50 rounded-3xl shadow-xl border-4 border-purple-400 dark:border-purple-600 transition-all duration-300 ${
          isShuffling ? 'transform -rotate-[18deg] translate-x-[-10px]' : 'transform -rotate-[12deg]'
        }`}
        style={{ top: '-15px', left: '-45px', zIndex: 1 }} 
      />
      <div 
        className={`absolute w-full h-full bg-gradient-to-br from-purple-100 to-purple-200 dark:from-purple-800/70 dark:to-purple-700/60 rounded-3xl shadow-xl border-4 border-purple-300 dark:border-purple-500 transition-all duration-300 ${
            ...
            <svg viewBox='0 0 500 500' xmlns='http://www.w3.org/2000/svg' className='w-full h-full'>
              <defs>
                <linearGradient id='blueGradient' x1='20%' y1='80%' x2='80%' y2='20%'>
                  <stop offset='0%' className='text-primary-500 dark:text-primary-600' stopColor='currentColor' stopOpacity='0.9' />
                  <stop offset='40%' className='text-blue-400 dark:text-blue-500' stopColor='currentColor' stopOpacity='0.85' />
                  <stop offset='100%' className='text-cyan-400 dark:text-cyan-600' stopColor='currentColor' stopOpacity='0.8' />
                </linearGradient>
              </defs>
              {/* Fan/Petal shape spreading from bottom left */}
              <path
                fill='url(#blueGradient)'
                d='M 50 450 Q 150 350 280 320 Q 400 300 480 340 Q 500 380 470 420 Q 440 460 380 480 Q 280 500 180 490 Q 100 480 50 450 Z
                   M 60 440 Q 140 380 240 360 Q 320 350 380 380 L 320 420 Q 240 400 160 430 Z
                   M 80 430 Q 150 390 220 380 Q 280 375 320 395 L 280 420 Q 220 410 160 425 Z'
              />
            </svg>
          </div>
        </div>
