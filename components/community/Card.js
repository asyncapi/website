...
export default function SmallHomeCards({icon, tagline, taglineBg, type="large", heading, description, bg, btnText, btnBg, link}) {
      ...
      <Link href={link} target="_blank">
        <a target={link.includes('http') && '_blank'}>
          <div
            ...
            <div className="dark:text-black p-2 rounded-xl bg-gray-100 text-center w-min text-xs flex justify-between">
  ...
  return (
    ...
    data-testid="Card-lg-bg">
      <div
        className={`dark:text-black p-2 rounded-xl text-center w-min text-xs flex justify-between ${taglineBg}`}
