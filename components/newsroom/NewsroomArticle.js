  ...
  return (
    <ul className="flex flex-col gap-2 w-full px-2 pb-4">
      { articlesData.map((article, index) => (
        <li key={ index }>
          <a
            className="dark:bg-slate-700 bg-white block p-6 rounded-md mb-2 shadow-md border border-gray-200 transition-all duration-300 ease-in-out hover:shadow-lg lg:w-full text-left"
            href={ article.url }
            target="_blank"
