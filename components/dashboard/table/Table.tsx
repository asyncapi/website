...
export default function Table({ title, data, className, listClassName }: TableProps) {
  return (
    <div
      className={`rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-dark-card shadow-lg dark:shadow-xl overflow-hidden lg:w-1/2 transition-all duration-300 hover:shadow-xl dark:hover:shadow-2xl flex flex-col ${className}`}
    >
            ...
            <div className='flex flex-col items-center justify-center py-8'>
              ...
              <ul className='mt-4 space-y-3 text-sm text-left max-w-md'>
                ...
                <li className='flex items-start gap-2'>
                  <span className='text-primary-500 dark:text-primary-400 mt-1'>•</span>
                  <span className='dark:text-gray-300'>
                    In the <span className='font-semibold text-green-700 dark:text-green-400'>#11_contributing</span>{' '}
                    channel, call out the maintainers that you want to work with. Ask them if there are any issues you
                    could solve. You know who these people are from{' '}
                    <span className='font-bold dark:text-white'>CODEOWNERS</span> file in each repo.
                  </span>
                </li>
                <li className='flex items-start gap-2'>
                  <span className='text-primary-500 dark:text-primary-400 mt-1'>•</span>
