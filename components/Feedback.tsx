  ...
  if (submitted) {
    return (
      <div className={`flex flex-col rounded-md border border-gray-200 p-4 text-center shadow-md ${className}`}>
        ...
        <a href='https://github.com/asyncapi/website' target='_blank' rel='noopener noreferrer' className='mx-auto mt-4 w-full rounded-md bg-black p-2 shadow-md transition-all duration-500 ease-in-out hover:shadow-lg md:w-1/2'>
    ...
    <div className={`flex flex-col rounded-md border border-gray-200 p-4 shadow-md ${className}`}>
      ...
      <form onSubmit={handleSubmit}>
        <div className='my-4 flex flex-col'>
          <textarea className='inline-block h-20 w-full rounded-md border bg-gray-50 px-2 py-1 align-top text-sm text-gray-700 focus:border-0' placeholder='Write your suggestions here' onChange={(e) => setFeedback(e.target.value)} required />
          <div className='mt-4 block text-sm lg:flex lg:flex-row'>
            <button className='w-full rounded bg-primary-500 py-2 text-white shadow-md transition-all duration-500 ease-in-out hover:shadow-lg lg:w-6/12' type='submit'>Submit feedback</button>
