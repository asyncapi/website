  ...
  return (
    <div className='fixed inset-x-0 top-0 z-60 max-h-full origin-top-right overflow-y-auto py-2 transition lg:hidden'>
      <div className='rounded-lg shadow-lg'>
        <div className='shadow-xs divide-y-2 divide-gray-50 rounded-lg bg-white dark:divide-gray-700 dark:bg-gray-900'>
          ...
          <div className='space-y-2 px-5 py-2' onClick={() => showMenu('learning')} data-testid='MobileNav-docs'>
            <h4 className='flex justify-between font-medium text-gray-800 dark:text-gray-200'>
              {' '}
          ...
          <div className='space-y-2 px-5 pt-2' onClick={() => showMenu('others')} data-testid='MobileNav-others'>
            <div className='grid gap-4'>
              <div>
                <h4 className='mb-4 flex justify-between font-medium text-gray-800 dark:text-gray-200'>
                ...
                {open === 'others' &&
                  otherItems.map((item: MenuItem, index: number) => (
                    <Link
                      ...
                      className='mb-4 block rounded-lg py-1 text-base font-medium leading-6 text-gray-700 transition duration-150 ease-in-out hover:bg-gray-50 dark:hover:bg-gray-800'
                ...
                {open === 'language' &&
                  uniqueLangs.map((lang) => (
                    <button
                      ...
                      className={`mb-4 ml-2 block w-full rounded-lg py-1 text-start text-sm font-medium leading-6 text-gray-700 transition duration-150 ease-in-out hover:bg-gray-50 dark:hover:bg-gray-800 ${currentLanguage.toLowerCase() === lang.text.toLowerCase() ? 'text-secondary-500' : ''}`}
