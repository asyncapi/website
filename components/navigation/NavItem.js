...
export default function NavItem ({
  ...
  if (href && !hasDropdown) {
    return (
      <Link href={href}>
        <a target={target} rel="noopener noreferrer" className={`${className} font-body text-base leading-6 font-semibold text-gray-700 hover:text-gray-900 focus:outline-none focus:text-gray-900 transition ease-in-out duration-150 ${router.pathname.substring(0,href.length) === href ? "text-black" :"text-gray-700"}`}>
          {text}
        </a>
      </Link>
    )
