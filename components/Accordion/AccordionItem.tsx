...
export default function AccordionItem({ itemIndex, title, content, isActive, setActiveIndex }: AccordionItemProps) {
  ...
  return (
    <div className='my-2 flex size-full flex-col gap-1 border border-gray-200 bg-white dark:bg-black dark:text-white px-2'>
      <button className='flex h-8 w-full items-center justify-between rounded-sm py-2' onClick={handleClick}>
        <div className='font-body font-semibold text-gray-800 dark:text-white antialiased'>{title}</div>
      ...
      {isActive && (
        <div className='rounded-sm border-t border-gray-200 py-2 font-body font-regular dark:text-white text-gray-700 antialiased'>
          {content}
        </div>
      )}
    </div>
