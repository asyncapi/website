    ...
    <li className={classNames}>
      <div className="flex">
        ...
        <div className="flex flex-1 shadow-sm rounded-md">
          <a href={item.html_url} target="_blank" rel="noopener noreferrer nofollow" className={`flex-shrink-0 flex items-center justify-center w-4 text-sm font-medium rounded-l-md ${colorClass}`}></a>
          <div className="flex-1 flex items-center justify-between border-t border-r border-b border-gray-200 bg-white rounded-r-md">
            <div className="flex flex-1 px-4 py-2 text-sm">
              ...
              <a href={item.url || item.html_url} target="_blank" rel="noopener noreferrer nofollow" className="block text-gray-900 font-medium hover:text-gray-600">{item.title}</a>
            </div>
            { isCollapsible && (
