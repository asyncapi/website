...
export default function SlackMessage({
  ...
  return (
    <div className={`my-2 flex pl-2 text-left ${className}`} data-testid='SlackMessage-main-div'>
      ...
      <div>
        <div className='-mt-1 text-sm font-bold'  data-testid='SlackMessage-name'>{name}</div>
        ...
        <div className='mt-0.5'>
          {
            reactions.map((reaction, index) => (
              <div key={index} className={`mr-1 inline rounded-xl px-2 py-0.5 ${reaction.mine ? 'border border-blue-500 bg-blue-50' : 'bg-gray-100'}`} data-testid='SlackMessage-reaction'>
                { reaction.icon ? (<img className='-mt-0.5 inline-block size-4 object-contain' src={reaction.icon} alt={reaction.name} data-testid='reactionIcon' />) : (<span className='mr-1 text-xs' data-testid='SlackMessage-span'>{reaction.emoji}</span>) }
                <span className='ml-1 inline-block text-xs font-bold text-blue-500' data-testid='SlackMessage-count'>{reaction.count}</span>
              </div>
            ))
          }
        </div>
