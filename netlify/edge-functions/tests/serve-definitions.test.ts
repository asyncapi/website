...
Deno.test('serve-definitions test for various response statuses', async () => {
  const testCases = [
    {
      requestURL: 'https://asyncapi.com/definitions/2.4.0/info.json',
      status: 200,
      mockParam: 'GET@https://asyncapi.com/definitions/2.4.0/info.json',
    },
    {
      requestURL: 'https://asyncapi.com/definitions/2.4.0/info.json',
      status: 304,
  ...
  for (const { requestURL, status, mockParam } of testCases) {
    ...
    mf.mock('*', () => {
      return new Response(
        status === 200 ? JSON.stringify({ url: requestURL }) : null,
        {
          status,
        },
      );
  ...
  assertEquals(
    metricCalls,
    testCases.filter(
      (testCase) => testCase.status === 200 || testCase.status === 304,
    ).length,
  );
});
...
Deno.test(
  'serve-definitions test for schema-related non-JSON requests',
  async () => {
    setup();
    metricCalls = 0;
    ...
    for (const requestURL of schemaRelatedNonJsonRequests) {
      ...
      console.log('Testing: ' + requestURL);
      const request = new Request(requestURL, { method: 'GET' });
      const response = await serveDefinitions(
        request,
