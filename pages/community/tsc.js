...
function addAdditionalUserInfo(user) {
	...
	if (userData.linkedin) userData.linkedin = `https://www.linkedin.com/in/${userData.linkedin}`;
...
export default function TSC() {
	...
	const tscMembers = sortBy(TSCMembersList.map((user) => addAdditionalUserInfo(user)), [ 'name' ]).filter(
		(user) => user.isTscMember
	);
	...
	return (
		<GenericLayout title="Technical Steering Committee" description={description} image={image} wide>
			<div className="py-12 relative mx-auto px-4 sm:px-6 lg:px-8">
				<div
					className="grid max-w-xl my-0 mx-auto lg:grid-cols-3 lg:gap-8 lg:max-w-screen-xl"
				...
				>
					<div>
						...
						<p className="my-4 text-base text-gray-500 lg:text-center">
							The Technical Steering Committee (TSC) is responsible for the oversight of the AsyncAPI
							Initiative. Maintainers (aka committers) make decisions at the given repository/project
							level. The TSC helps to make decisions on a higher level, or when maintainers cannot find a
							consensus.
						</p>
					</div>
					<div>
						...
						<p className="my-4 text-base text-gray-500 lg:text-center">
							Anybody can become a member of the TSC. All you have to do is become a maintainer of one of
							the AsyncAPI projects! To become a maintainer, you just need to regularly contribute to one
							of the projects and then other maintainers will invite you to join. You can also build a
							great AsyncAPI-based project that we don't have yet in our GitHub organization and donate it
							(we'll ask you to stay as a maintainer). Follow this
							<TextLink
					...
					<div>
						...
						<p className="my-4 text-base text-gray-500 lg:text-center">
							...
							that gives power to the people actively involved and working on the project. No matter if
							you are an individual contributor or backed by a company, you have equal rights. Read{' '}
							<a
				...
				<div className="mt-10">
					...
					>
						{tscMembers.map((user) => <UserInfo key={user.github} user={user} />)}
						<QuestionCard />
					</ul>
				</div>
			</div>
		...
		>
			<div className="flex flex-row">
				...
				<div className="flex-1">
					<div className="font-bold text-lg my-3" data-testid="UserInfo-name">
						{user.name}
					</div>
	...
	if (user.availableForHire) {
		return (
			<div
				...
				className="inline-flex items-center px-3 py-1 rounded-full text-md font-medium leading-5 bg-green-100 text-green-800"
			>
				Available for hire
			</div>
		);
		...
		return (
			<div
				data-testid="status-element"
				className="inline-flex items-center px-3 py-1 rounded-full text-md font-medium leading-5 bg-orange-100 text-orange-800"
			>
				{user.company}
			</div>
		);
		...
		return (
			<div
				data-testid="status-element"
				className="inline-flex items-center px-3 py-1 rounded-full text-md font-medium leading-5 bg-blue-100 text-blue-800"
			>
				Individual Member
			</div>
		);
