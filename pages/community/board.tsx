 ...
 */
export default function Board({ data }: { data: Tsc[] | Ambassador[] }) {
  return (
    <CommunityLayout members={data} membership={Membership.BOARD}>
...
/**
 * Fetches static props for the board members page.
 */
export const getStaticProps: GetStaticProps = async () => {
  const filePath = path.join(process.cwd(), 'config', 'TSC_BOARD_MEMBERS.yaml');
  const fileContent = fs.readFileSync(filePath, 'utf8');
  const parsedData = yaml.load(fileContent);

  const mapped = Array.isArray(parsedData) ? parsedData.map((item: Tsc[] | Ambassador[]) => ({ ...item })) : parsedData;
