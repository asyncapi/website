...
Once that is done, you should create some new sub-directories to begin building your Java client.
  ...
  6. Create a new file named **index.js** in a `template` folder at the root directory. This file is used to define the **logic** for your template.
...
- **repositories** - This section tells your app to fetch dependencies from Maven Central to retrieve the MQTT client library.
...
Here is the sample code to paste into the `Client.java` file you generated above by running the `asyncapi generate fromTemplate src/fixtures/asyncapi.yml ./ --output src/main/java` command. 
...
export function TopicFunction({ operations }) {
  ...
  /*
   * This function returns a list of objects, one for each receive operation, each containing two properties: `name` and `topic`.
...
export default function ({ asyncapi, params }) {
    ...
    // Generate Java code for each operation dynamically using TopicFunction
...
public class Client {
    ...
    public static void main(String[] args) {
        ...
        //serviceClient.sendTemperatureRise("Sensor-1: 30°C");
    }
}`
    }
    </File>
...
> This example uses AsyncAPI v3 structure with operations instead of publish/subscribe.
...
operations:
  temperatureDrop:
    action: send
    channel:
      $ref: '#/channels/temperature/dropped'

  temperatureRise:
    action: send
    channel:
      $ref: '#/channels/temperature/risen'
```
