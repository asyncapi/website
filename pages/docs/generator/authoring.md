...
To work on a template, you need an AsyncAPI specification file that you can use for testing. We recommend you to use [this dummy file](test/docs/dummy.yml), given its purpose is to cover as many features of AsyncAPI as possible. Do not copy this file to your template but use it directly from this repo like this:

```generator-cli
ag https://bit.ly/generator-dummy ./your-template
```

> In case you find some features missing or other possible improvements in the dummy file, feel free to suggest changes. The goal is to build a file that all templates can use and check their specification features coverage.
...
For example, you can specify a filename like `$$channel$$.js` to generate a file for each channel defined in your AsyncAPI file. 
