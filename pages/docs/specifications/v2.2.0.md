...
<a name="serverObjectProtocol"></a>protocol | `string` | <LifeCycle text="required" />. The protocol this URL supports for connection. Supported protocol include, but are not limited to: `amqp`, `amqps`, `http`, `https`, `ibmmq`, `jms`, `kafka`, `kafka-secure`, `anypointmq`, `mqtt`, `secure-mqtt`, `stomp`, `stomps`, `ws`, `wss`, `mercure`.
...
<a name="schemaObjectDeprecated"></a> <LifeCycle text="deprecated" /> | `boolean` | Specifies that a schema is deprecated and SHOULD be transitioned out of usage. Default value is `false`.
