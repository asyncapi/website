...
For today's blog post about AsyncAPI Docs 📑, I wanted to share with all technical writers about our organization’s plan for participation in `Google Season of Docs 2022 (GSoD)`. 
...
We also have a need to re-structure specifically the [Generator tool](https://github.com/asyncapi/template-for-generator-templates) docs. Because this is one of our main tools, this in itself is also big enough to be it's own independent project for 2022 GSoD. Currently, our Generator docs need a major update, to better explain every single functionality of the Generator. 
...
### ⌛ Timeline
The project itself will take approximately 3-6 months to complete, depending on how many technical writers (TW) get involved. If we can get even just 2-4 TWs, then we can work on both migrating the CLI/Tools documentation and the Generator Docs. But if we can only get 1-2 folks, it would be hard to do more than the migration of CLI and Tools docs. Ideally, the timeline could look like the following:
