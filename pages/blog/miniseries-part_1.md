...
At the moment each of the AsyncAPI documents contains their own definition of the channels. But what if I where to add a new validation rule such as a new property to the **playerItemPickupPayload** schema? In this case I would have to change this for both applications which is way too much work :smile:
...
# What's next
Now that our API's are designed for our two applications, I can move on to Part 2: Implementing the applications using code generation.
