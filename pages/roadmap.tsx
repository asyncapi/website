...
const solutions: Record<string, Solution> = {
  createFramework: {
    ...
    description: (
      <>
        <p>
          We should create a framework that allows users to quickly create a new API. It should provide the following
          features, among others:
        </p>
        <ul>
          <li>
            The user should be able to create an API with just an AsyncAPI file and some files containing the business
            logic (not counting package.json files or similar).
          </li>
  ...
  createPluginsForOtherFrameworks: {
    ...
    description: (
      <>
        <p>
          Even though we want to create our own framework, some people are already using other frameworks and it's very
          unlikely they'll switch frameworks in all their codebases. Instead of forcing them to use our framework, we
          should offer plugins for the most common frameworks (when possible). E.g., Spring Boot, Spring Cloud Streams,
          Akka, etc.
        </p>
        <p>The plugin should allow them to get at least the following features, when possible:</p>
        <ul>
          <li>Automatically generate documentation.</li>
          <li>Automatically validate messages. This goes both ways, for the outgoing and the incoming messages.</li>
        </ul>
  ...
  makeGeneratorGenerateModels: {
    ...
    description: (
      <p>
        Currently, a Generator template author has to do a lot of work to generate types or data models. Translating
        JSON Schema to a programming language is a tough task. This should be handled by us instead of leaving it to the
        template author.
      </p>
    )
  },
  makeGeneratorSupportReact: {
    ...
    description: (
      <p>
        By supporting React in the Generator, it will become easier to create better templates, leading to a better user
        experience.
      </p>
    )
  },
  createOutstandingDocs: {
    ...
    description: (
      <>
        <p>
          The current documentation and tutorials are good already. However, they're not enough for most people to get
          started with AsyncAPI. This is usually to the fact that the specification is large and tough to read, there
          are not many examples of how to use AsyncAPI for your use case (or a similar one), and there's no guide on how
          to use AsyncAPI with each protocol.
        </p>
        <p>
          We should improve our documentation and tutorials to cover a full getting started guide, many different use
          cases, and examples of how AsyncAPI can be used with different protocols. From beginner to advanced, step by
          step. Our documentation should start looking a little bit more like a game: enjoyable and interactive.
        </p>
        <p>
          Video content would also be super helpful here as new generations tend to use more the video format than the
          written one. In any case, we need it written so it's easy to copy and paste.
        </p>
      </>
    )
  },
  ...
  solveChannelReusability: {
    ...
    description: (
      <p>
        Reusing channels across APIs is hard because the publish or subscribe verb is included in the channel
        definition. We should come up with a way to decouple channel definition from the operation (publish/subscribe).
      </p>
    )
  },
  solvePublishSubscribe: {
    ...
    description: (
      <p>
        Many people get confused with the "perspective" of the publish and subscribe verbs. In many cases, it's really
        awkward to work with the current definition/perspective.
      </p>
    )
  },
  createNewParserAPI: {
    ...
    description: (
      <p>
        Our current parser is tightly coupled to the AsyncAPI document structure. We should create a new parser API that
        represents intents instead of the structure of the document.
      </p>
    )
  },
  createOutstandingContributingGuide: {
    ...
    description: (
      <>
        <p>
          When a new potential contributor comes in, they don't know where to start looking or how to make their first
          contribution. We should create clear documentation on the website explaining how the project is structured. To
        ...
        <p>
          <strong>
            Ideally, each repo/project should have its own contributing guide that would be linked from the main one.
          </strong>
        </p>
      </>
    )
  ...
  createUILibrary: {
    ...
    description: (
      <p>
        We currently have an HTML template and a React component for rendering HTML documentation from an AsyncAPI
        document. We should unify both and offer a componentized version of the React component.
      </p>
    )
  },
  createEventGateway: {
    ...
    description: (
      <p>
        One way to prevent incompatible or "wrong-formed" messages arrive in the message broker is to place a proxy or
        gateway in the middle, between the client and the broker. This broker could leverage AsyncAPI definitions to
        validate the messages in real-time. It could happen before they're published, received, or both.
      </p>
    )
  },
  createSupportProgram: {
    ...
    description: (
      <>
        <p>
          We are very fortunate with the support we got at AsyncAPI. There are many people out there creating great
          stuff. Some of these things power AsyncAPI and others augment it, making it easier for the end-user to adopt
          AsyncAPI. We should create a program to give support to these community members. It should consist —at least—
...
const outcomes: Record<string, Outcome> = {
    ...
    description: (
      <>
        ...
        <ul>
          ...
          <li>
            We should avoid creating a road map that looks like a set of specific promises like "We're going to
            implement X" or "We're going to do Y". Instead, we should focus on what the user should be able to do with
            AsyncAPI once we accomplish a specific goal.
          </li>
          <li>
            We should also avoid giving a timeframe or time estimation. AsyncAPI is driven by the community so we can't
            (and shouldn't) put pressure on contributors to finish something before a specific deadline.
          </li>
        </ul>
      </>
    ),
  ...
  impossibleUnsyncDocsAndCode: {
    ...
    description: (
      <>
        <p>
          One common pain point when creating APIs (and software in general) is having to maintain the documentation up
          to date. There are lots of people (including myself) preaching design-first is the best approach. However,
          many people love the code-first approach and I think AsyncAPI should be above this conversation. So what if
          neither design-first nor code-first? Or what if it's actually a mix? We should find the right balance between
          the two so documentation and code are both somehow "linked" and impossible to get wrong. Or said another way,
          if documentation is wrong, code is also wrong.
        </p>
        <p>
          Of course, I'm not referring to the human-readable fields of the spec like description or summary but the ones
          that are machine-readable.
        </p>
      </>
    ),
    solutions: [solutions.createFramework, solutions.createPluginsForOtherFrameworks]
  },
  userIsAbleToFocusOnlyBusinessLogic: {
    ...
    description: (
      <>
        <p>
          Ideally, the user should not be focusing on anything that's not related to their business logic. This includes
          learning the AsyncAPI specification. Nobody <strong>should be forced</strong> to learn the AsyncAPI spec to
          create a basic API. However, it doesn't mean the user should not be able to improve or create more complex
          APIs if they learn AsyncAPI and its tooling.
        </p>
        <h2>Scope</h2>
        <ul>
          <li>
            An AsyncAPI file can be created without having to understand AsyncAPI syntax. This can be a CLI tool that
            asks questions, an application UI, a form, etc.
          </li>
          <li>Custom logic can be triggered when a message is received on a channel.</li>
          <li>
            Custom logic that sends a message can be triggered as the result of receiving a message, on startup, or any
            other application lifecycle event.
          </li>
    ...
    solutions: [solutions.makeGeneratorSupportReact, solutions.makeGeneratorGenerateModels]
  },
  userIsAbleToCreateAPIInUnder20Minutes: {
    ...
    description: (
      <p>
        This outcome is all about the best-in-class user experience. We want people to create APIs without having to
        understand AsyncAPI and in under 20 minutes. All of this process should —of course— be powered by the AsyncAPI
        specification as the intermediary language.
      </p>
    ),
    solutions: [solutions.createCLI, solutions.createFramework, solutions.createOutstandingDocs]
  },
  specificationAndParsersReadyForFuture: {
    ...
    description: (
      <>
        ...
        <ol>
          <li>
            Publish and subscribe verbs are not clear and most people get confused. It's also very probable we end up
            adding more verbs to accommodate requests, responses, queries, mutations, etc.
          </li>
          <li>
            Reusing channels across APIs is hard because the <code>publish</code> or <code>subscribe</code> verb is
            included in the channel definition.
          </li>
        </ol>
        <p>
          Before we proceed with adding more features to the spec, we should fix these structural problems. Otherwise,
          changing the spec afterward is going to be a mess.
        </p>
      </>
    ),
    solutions: [solutions.createNewParserAPI, solutions.solvePublishSubscribe, solutions.solveChannelReusability]
  },
  timeFirstContribution1Day: {
    title: 'Time to first contribution goes down to a day',
    description: (
      <p>
        Currently, it's hard for a developer to make their first contribution. We should make it super easy for anyone
        to contribute. Ideally, the time to the first contribution goes down to a day or less since they decided to
        contribute.
      </p>
    ),
  ...
  userIsAbleRenderDocsInsideTheirDocs: {
    ...
    description: (
      <p>
        We currently provide a way to generate documentation, either via the HTML template or the React component.
        However, some people want to embed only specific parts of the docs inside their docs as opposed to the full-page
        docs we offer right now. We should enable users to accomplish this using our tools.
      </p>
    ),
  ...
  reuseOtherSchemaDefsInsideAsyncAPI: {
    ...
    description: (
      <p>
        It is common to find the same data structures defined using multiple schema/type definition languages, i.e.,
        JSON Schema, Avro, Protobuf, GraphQL types, etc. If this outcome is achieved, users should be able to link
        and/or embed their definitions using any of the following languages: OpenAPI Schema, RAML Data Type, Avro,
        GraphQL Type, Protobuf, and XSD.
      </p>
    )
  },
  usersCanModifyExistingAppsToWorkWithAsyncAPI: {
    ...
    description: (
      <p>
        Even though we're building our own framework, people should not be forced to leave their frameworks of choice to
        benefit from AsyncAPI features like automatic documentation generation and message validation.
      </p>
    ),
  ...
  onlyWellFormedMessagesReachBroker: {
    ...
    description: (
      <>
        <p>
          When doing EDAs, there is a common problem that repeats across many companies, especially the big ones. People
          are not able to answer the question "who is subscribing to this topic/channel?". This management problem
          derives into a more serious one: someone makes a breaking change on the information sent on one channel and
          others break because they were not aware of this change. Hopefully, the subscribers break in a "noisy" way so
          you can detect the error and fix it. Other people opt for not pushing breaking changes into an existing
          channel and decide to create a new one, leading to a pile of unused channels over time.
        </p>
        <p>The problem I just described actually consists of two parts:</p>
        <ol>
          <li>
            Lack of discovery tools for EDAs. E.g., being able to answer the question "who is subscribing to this
            channel?"
          </li>
  ...
  numberContributorsAbove100: {
    ...
    description: (
      <p>We make top repos grow to a point where we go over 100 contributors in the whole history of the project.</p>
    ),
    solutions: [solutions.createOutstandingContributingGuide, solutions.createSupportProgram]
  },
  numberProjectsAbove100: {
    title: 'The number of community and community-donated projects go above 100',
    description: (
      <p>
        We already have a bunch of tools and spec-related repos. This outcome will be done when we're over 100. That
        said, we shouldn't accept projects just for the sake of increasing that number.
      </p>
    ),
  ...
  yearlyNumberPresentersWritersAbove50: {
    ...
    description: (
      <p>
        A community is not made of programmers only. People producing content in form of blog posts, videos, live
        streams, podcasts, and conference talks are super important as well. We should help to make it grow. The more
...
const roadmapData: RoadmapData = {
  outcomes: {
    ...
    later: [outcomes.timeFirstContribution1Day, outcomes.userIsAbleRenderDocsInsideTheirDocs],
    future: [
          ...
          <div className='relative'>
            ...
            <div className='relative mt-20'>
              <div className='grid lg:grid-cols-3 lg:gap-8'>
                <div className='border border-gray-200 rounded-lg shadow-md bg-white p-9'>
                ...
                <div className='border border-gray-200 rounded-lg shadow-md bg-white p-3'>
                  <div className='mb-2 mt-6 lg:my-0 lg:text-center'>
                    <Paragraph
                      typeStyle={ParagraphTypeStyle.md}
                      textColor='text-secondary-500'
